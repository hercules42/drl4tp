...
def plot_data(gb_plot, pg_plot, ac_plot, mpnn_plot, nognn_plot):
    ...
    ax[0].plot(range(len(gb_plot)), gb_plot, marker='', label='w/ greedy rollout baseline')
    ax[0].plot(range(len(ac_plot)), ac_plot, marker='', label='w/ learnable critic', color='red')
    ax[0].plot(range(len(pg_plot)), pg_plot, marker='', label='w/o baseline', color='green')
    ...
    ax[0].set_ylabel('Validation optimality gap', fontsize=20)
    ax[0].grid(True)

    mpnn_plot = np.insert(mpnn_plot, 0, np.nan)
    nognn_plot = np.insert(nognn_plot, 0, np.nan)

    ax[1].plot(range(len(gb_plot)), gb_plot, marker='', label='w/ HGNN')
    ...
    ax[1].set_ylabel('Validation optimality gap', fontsize=20)
    ax[1].grid(True)
    ...
    fig.savefig('two_curve.pdf', format='pdf')
