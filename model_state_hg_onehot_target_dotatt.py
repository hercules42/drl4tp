...
"""
before: have graph state in pointer net
now: replace graph state with target embedding
"""
...
class scaledDotProductAttention(nn.Module):
    ...
    def forward(self, static_hidden, decoder_hidden):
        """
        Args:
            query --- decoder_hidden: (batch_size, 1, hidden_size)
            keys ---static_hidden: (batch_size, seq_len, hidden_size)"""
        static_hidden  = self.W(static_hidden)
        decoder_hidden = self.W(decoder_hidden)
        scores = torch.bmm(decoder_hidden, static_hidden.transpose(-2, -1)) / math.sqrt(self.hidden_size)  # (batch_size, 1, seq_len)
...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, decoder_hidden):
        ...
        attns = F.softmax(attns, dim=2)  # (batch, 1,seq_len)        #这两行是公式4
        return attns
...
class Pointer(nn.Module):
    ...
    def forward(self, static_hidden,decoder_hidden, target_hidden, last_hh):
        ...
        attn_weights = self.attn(static_hidden, rnn_out)
        context = attn_weights.bmm(static_hidden)  # (B, 1, num_feats)

        # Calculate the next output using Batch-matrix-multiply ops
        context = context.expand_as(static_hidden)
        target_hidden = target_hidden.expand_as(static_hidden)
        energy = torch.cat((static_hidden, context, target_hidden), dim=2)  # (B, num_feats*3, seq_len)
...
class DRL4TP(nn.Module):
    ...
    def forward(self, static,weight, last_hh=None, actions=None):  ###
        ...
        for step_idx in range(max_steps):
            ...
            probs, last_hh = self.pointer(node_embedding, decoder_hidden, target_hidden, last_hh)
            probs = torch.where(mask<=0,torch.tensor(-1*np.inf,dtype=torch.float,device=device),probs)
            ...
            state = torch.cat((static.transpose(2,1),mask_state), dim=-1)
            node_embedding = self.gnn_embedding(state, adj)
            #target_embddding = node_embedding[:,-1,:].unsqueeze(1)
