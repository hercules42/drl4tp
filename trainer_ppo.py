...
from model_ppo_state_hg_onehot_target import DRL4TP, Encoder
...
def validate(data_loader, actor, reward_fn, render_fn=None, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            tour_indices, _ , mask,_ = actor.forward(static, weight)  ###
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    lr_decay_step = 39070  # 相当于每个epoch更新一次
    lr_decay_rate = 0.85
    ...
    for epoch in range(15):                  # 本来是10
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                #相当于采样
                old_tour_indices, old_tour_logp, mask, _ = actor(static,weight)  ###
                #old_tour_indices = torch.cat((torch.zeros(static.size(0),1,device=device,dtype=int), old_tour_indices), dim=1)
                print(old_tour_logp[0])
            ...
            actor.eval()
            C_tour_indices, _, C_mask,_ = actor(static,weight)  ###
            ...
            for ep_idx in range(epochs_ppo):
                ...
                _, _, mask, all_logp = actor(static,weight)  ###
                #if ep_idx <=1:
                #    print(tour_logp[0])

                gather_tour_logp = all_logp.gather(2,old_tour_indices.unsqueeze(-1))  # [batch_size, num_nodes]
                gather_tour_logp = gather_tour_logp.squeeze(2)
                print(gather_tour_logp[0])
                gather_tour_logp = gather_tour_logp.sum(dim=1)
                
                print(old_logp[0])
                ratio = torch.exp(gather_tour_logp - old_logp)
...
def train_tp(args):
    ...
    test_data = TPDataset(args.num_nodes, 1000, args.seed + 2)
