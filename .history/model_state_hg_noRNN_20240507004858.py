...
class Pointer(nn.Module):
    ...
    def forward(self, static_hidden, state_embedding, decoder_hidden, target_hidden):

        # print(self.v)
        #print("decoder_hidden",decoder_hidden)
        rnn_out= decoder_hidden
...
class DRL4TP(nn.Module):
    ...
    def __init__(self, static_size,hidden_size, update_fn=None, num_layers=1, dropout=0., seed=42):
        ...
        self.gnn_embedding = DirectedHGNNet(static_size+1, hidden_size)
    ...
    def forward(self, static,weight, last_hh=None, actions=None):  ###
        ...
        #  1: 没涉及到的点
        # 0: 被选中的点
        # 3: 被影响到但是没有发出信号（不在路径中）
        # -2: 上一步中被选择的点

        # 一开始选择第一个点
        mask[:,0] = -2 
        state = torch.cat((static.transpose(2,1),mask.unsqueeze(-1)), dim=-1)
        ...
        for step_idx in range(max_steps):
            ...
            probs= self.pointer(node_embedding,state_embedding, decoder_hidden, target_hidden)
            #print("probs",probs)
            log = mask.log()
