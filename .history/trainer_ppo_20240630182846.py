    ...
    for epoch in range(15):                  # 本来是10
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                tour_indices, old_tour_logp, mask, _ = actor(static,weight)  ###
                tour_indices = torch.cat((torch.zeros(static.size(0),1,device=device,dtype=int), tour_indices), dim=1)
            ...
            for ep_idx in range(epochs_ppo):
                ...
                gather_tour_logp = all_logp.gather(2,tour_indices.unsqueeze(-1))  # [batch_size, num_nodes]
                gather_tour_logp = gather_tour_logp.squeeze(2)
                print(gather_tour_logp.shape)
                print(tour_logp.shape)
