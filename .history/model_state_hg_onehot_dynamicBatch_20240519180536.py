...
class DRL4TP(nn.Module):
    ...
    def forward(self, static,weight, last_hh=None, actions=None):  ###
        ...
        #每一步把已经结束的样本的轨迹和概率存到下面两个向量里
        end_flag = torch.ones(batch_size,device=device) #等于0表示已经结束
        
        tour_idx_return = torch.ones(batch_size,sequence_size,device=device)*(sequence_size-1)
        tour_logp_return = torch.ones(batch_size,sequence_size,device=device,dtype=torch.float)
        curr_index = torch.tensor(np.arange(batch_size),dtype=torch.long,device=device)
        ...
        for step_idx in range(max_steps):
            ...
            #每一步训练的时候应该只训练当前没结束的点，维护它的索引
            curr_index = torch.nonzero(end_flag==1)
            curr_static = static[curr_index]
            curr_mask = mask[curr_index]
            curr_node_embedding = node_embedding[curr_index]
            curr_state_embdding = state_embedding[curr_index]
            curr_decoder_hidden = decoder_hidden[curr_index]
            curr_target_hidden = target_hidden[curr_index]
            curr_last_hh = last_hh[curr_index]
            
            probs, curr_last_hh = self.pointer(curr_node_embedding,curr_state_embdding, curr_decoder_hidden, curr_target_hidden, curr_last_hh)
            #probs的size(0)是end_flag==1的个数
            ...
            #print("probs",probs)
            probs = F.softmax(probs + log, dim=1)    #长度是目前未完成的序列的数目
            ...
            end_flag[...] = 0
            curr_index = torch.nonzero(chosen_idx!=sequence_size-1)
