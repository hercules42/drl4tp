...
from model_state_hg_onehot_target_noRNN import DRL4TP, Encoder
...
def train(actor, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    now = now.replace(':', '_')
    save_dir = os.path.join(task, 'hg_state_onehot_target_%d' % num_nodes, now[:5])
