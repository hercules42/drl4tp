...
from model_state_hg_onehot import DRL4TP, Encoder
...
# 实现PPO算法
class PPO:
    def __init__(self, actor, critic, clip_param, ppo_epoch, num_mini_batch,
                 value_loss_coef, entropy_coef, lr=None, eps=None, max_grad_norm=None):
        self.actor = actor
        self.critic = critic
        ...
        self.optimizer = optim.Adam(actor.parameters(), lr=lr, eps=eps)
    ...
    def update(self, rollouts):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                values, action_log_probs, dist_entropy, _ = self.actor.evaluate_actions(
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(15):                  # 本来是10
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            weight = tp.weight_fn(static)
            # 邻接矩阵
            neighbour_mask = (weight>0) & (weight < 10)   
            adj = weight.clone()
            adj[neighbour_mask] = 1
            adj[~neighbour_mask] = 0
            adj = adj.transpose(1,2)
            ...
            with torch.no_grad():
                ...
                tour_indices, old_tour_logp, mask = actor(static,adj,weight, x0, xt)  ###
                ...
                advantage = (reward - critic_est)/num_nodes
                advantage = advantage-advantage.mean()
                advantage = advantage/(advantage.std()+1e-8)
                #为什么一开始的时候advantage.mean()为0????????
                #print(advantage)

            for ep_idx in range(epochs_ppo):

                tour_indices, tour_logp, mask = actor(static,adj,weight, x0, xt, actions=tour_indices)
                #if ep_idx <=1:
                #    print(tour_logp[0])

                ratio_step = torch.exp(tour_logp-old_tour_logp)
